...
def send_text(post):
    ...
    if bday.replace('/', '-') in time and resp.get("likes", "None") == "None":
        ...
        if "happy" in message or "bday" in message or "birthday" in message or (
                "many" in message and "returns" in message):
...
user = 'me'
# ENTER YOUR BDAY IN MM/DD FORMAT
bday = ""
while len(bday) != 5:
    bday = input("Enter Your Birthday in MM/DD Format! Ex: 22nd feb will be 02/22:")
    # bday = '01/01'
    if len(bday) != 5:
...
while True:
    try:
        ...
        # Facebook.
        # print(posts)
        [send_text(post=post) for post in posts['data']]
